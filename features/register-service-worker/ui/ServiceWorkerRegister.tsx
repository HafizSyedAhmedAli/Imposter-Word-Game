...
/**
 * Moved from `components/pwa/ServiceWorkerRegister.tsx` as the
 * `features/register-service-worker` slice (FSD migration step 6) --
 * this is a self-contained capability (register the SW, surface an
 * "update ready" banner) that doesn't compose with any other screen
 * piece, so it's a `features/` slice with a `ui/` segment rather than a
 * `widgets/` concern. Logic and JSX unchanged; only its folder and
 * import path moved. The one external consumer, `app/layout.tsx`, has
 * been repointed to `@/features/register-service-worker`.
 */
